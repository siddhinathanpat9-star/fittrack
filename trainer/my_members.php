...
// Get assigned members with details
$members = [];
try {
    $sql = "SELECT 
                u.id, u.full_name, u.email, u.phone, u.status as user_status,
                m.membership_type, m.membership_start, m.membership_end,
                DATEDIFF(m.membership_end, CURDATE()) as days_left,
                m.fitness_goals, m.emergency_contact, m.emergency_phone,
                (SELECT COUNT(*) FROM attendance WHERE user_id = u.id) as total_attendance,
                (SELECT COUNT(*) FROM attendance WHERE user_id = u.id AND date = CURDATE()) as attended_today,
                (SELECT COUNT(*) FROM workout_plans WHERE member_id = u.id) as workout_plans
            FROM users u
            JOIN members m ON u.id = m.user_id
            WHERE m.assigned_trainer_id = :trainer_id";
...
foreach ($members as $member) {
    if ($member['days_left'] > 0 && $member['days_left'] <= 7) {
        $expiring_soon++;
    }
...
$page_title = 'My Members - ' . APP_NAME;
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="page-header d-flex justify-content-between align-items-center">
                            <h1><i class="fas fa-users"></i> My Members <small>View and manage your assigned members</small></h1>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5><i class="fas fa-list"></i> Assigned Members</h5>
                    </div>
                    <div class="card-body p-0">
                        <?php if (empty($members)): ?>
                            <div class="text-center py-5">
                                <i class="fas fa-users fa-4x text-muted mb-3"></i>
                                <h5 class="text-muted">No members assigned yet</h5>
                                <?php if (!empty($search)): ?>
                                    <p class="text-muted">Try adjusting your search</p>
                                    <a href="my_members.php" class="btn btn-primary"><i class="fas fa-times"></i> Clear Search</a>
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge badge-info"><?php echo ucfirst($member['membership_type']); ?></span>
                                            </td>
                                            <td>
                                                <?php
                                            ...
                                            <td>
                                                <span class="badge badge-secondary" title="Total attendance"><?php echo $member['total_attendance']; ?></span>
                                                <?php if ($member['attended_today']): ?>
                                                    <span class="badge badge-success" title="Checked in today"><i class="fas fa-check"></i></span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <span class="badge badge-info"><?php echo $member['workout_plans']; ?> plans</span>
                                            </td>
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a href="view_member.php?id=<?php echo $member['id']; ?>" class="btn btn-sm btn-info" title="View"><i class="fas fa-eye"></i></a>
                                                    <a href="mark_attendance.php?member_id=<?php echo $member['id']; ?>" class="btn btn-sm btn-success" title="Mark Attendance"><i class="fas fa-clock"></i></a>
                                                    <a href="create_workout.php?member_id=<?php echo $member['id']; ?>" class="btn btn-sm btn-primary" title="Create Workout"><i class="fas fa-dumbbell"></i></a>
                                                    <a href="message_member.php?member_id=<?php echo $member['id']; ?>" class="btn btn-sm btn-warning" title="Send Message"><i class="fas fa-envelope"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
