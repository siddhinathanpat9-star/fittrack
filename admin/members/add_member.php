...
// Get all active trainers for assignment dropdown
$trainers = [];
try {
...
$membership_plans = [];
try {
    $stmt = $pdo->query("SELECT * FROM membership_plans WHERE status = 'active' ORDER BY price");
    $membership_plans = $stmt->fetchAll();
} catch (Exception $e) {
    // If membership_plans table doesn't exist, use default plans
    $membership_plans = [
        ['id' => 'basic', 'name' => 'Basic', 'price' => 29.99, 'duration_days' => 30],
        ['id' => 'premium', 'name' => 'Premium', 'price' => 49.99, 'duration_days' => 30],
        ['id' => 'vip', 'name' => 'VIP', 'price' => 79.99, 'duration_days' => 30]
    ];
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $membership_end = $_POST['membership_end'] ?? date('Y-m-d', strtotime('+1 month'));
        ...
        $stmt = $pdo->prepare("
            INSERT INTO members (
                user_id, membership_type, membership_start, membership_end, 
                assigned_trainer_id, height, weight, fitness_goals, 
        ...
        if (!$stmt->execute([
            $user_id, $membership_type, $membership_start, $membership_end, 
            $assigned_trainer, $height, $weight, $fitness_goals, 
            $emergency_contact, $emergency_phone
        ])) {
...
$page_title = 'Add Member - ' . APP_NAME;
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <ul class="list-unstyled components">
                ...
                <li class="active">
                    ...
                    <ul class="collapse show list-unstyled" id="membersSubmenu">
                        ...
                        <li class="active"><a href="add_member.php"><i class="fas fa-user-plus"></i> Add Member</a></li>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="page-header">
                            <h1><i class="fas fa-user-plus"></i> Add New Member <small>Create a new gym member account</small></h1>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5><i class="fas fa-user-edit"></i> Member Information</h5>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="" id="addMemberForm" class="needs-validation" novalidate>
                            ...
                            <div class="row mt-3">
                                <div class="col-12">
                                    <h6 class="text-success">Membership Information</h6>
                                    <hr>
                                </div>
                                ...
                                <div class="col-md-4 form-group">
                                    ...
                                    <select class="form-control" id="membership_type" name="membership_type" required>
                                        <?php foreach ($membership_plans as $plan): ?>
                                            <option value="<?php echo is_array($plan) ? $plan['id'] : $plan->id; ?>">
                                                <?php echo is_array($plan) ? $plan['name'] : $plan->name; ?> 
                                                ($<?php echo is_array($plan) ? $plan['price'] : $plan->price; ?>)
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-4 form-group">
                                    <label for="membership_start">Start Date</label>
                                    <input type="date" class="form-control" id="membership_start" name="membership_start" 
                                           value="<?php echo date('Y-m-d'); ?>">
                                </div>
                                
                                <div class="col-md-4 form-group">
                                    <label for="membership_end">End Date</label>
                                    <input type="date" class="form-control" id="membership_end" name="membership_end" 
                                           value="<?php echo date('Y-m-d', strtotime('+1 month')); ?>">
                                </div>
                                
                                <div class="col-md-6 form-group">
                                    <label for="assigned_trainer">Assign Trainer</label>
                            ...
                            <div class="row mt-3">
                                <div class="col-12">
                                    <h6 class="text-info">Health Information</h6>
                                    <hr>
                                </div>
                                ...
                                <div class="col-md-6 form-group">
                                    ...
                                    <textarea class="form-control" id="fitness_goals" name="fitness_goals" rows="2" 
                                              placeholder="e.g., Lose weight, build muscle, improve endurance"><?php echo htmlspecialchars($_POST['fitness_goals'] ?? ''); ?></textarea>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <a href="manage_members.php" class="btn btn-secondary btn-lg">
