<?php
/**
 * admin/view_user.php - View user details (Auto-adapting version)
 */
...
try {
    ...
    if (!$user) {
        ...
        if ($user['user_type'] == 'member') {
            // Fetch member record (link via user_id)
            ...
            if ($member) {
                ...
                // Determine foreign key columns for payments, attendance, class_bookings
                ...
                $payments_stmt->execute([$user['id']]); // always use user id
                ...
                // ========== FIXED: Fetch class bookings with dynamic schedule table detection ==========
                $recent_bookings = [];
                $scheduleTable = null;
                ...
                // No error – we'll simply show a message in the Classes tab if empty
            } else {
            ...
            if ($trainer) {
                $user = array_merge($user, $trainer);
                
                // Determine foreign key for classes table
                $classes_fk = getForeignKey($pdo, 'classes');
                
                // Fetch classes assigned to this trainer
                $classes_stmt = $pdo->prepare("SELECT * FROM classes WHERE $classes_fk = ? ORDER BY class_id DESC");
                $classes_stmt->execute([$user['id']]);
                $assigned_classes = $classes_stmt->fetchAll();
            } else {
                $error = "Trainer record not found for this user.";
            }
        }
    }
} catch (Exception $e) {
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="page-header">
                            <h1><i class="fas fa-user-circle"></i> User Details <small>Viewing user #<?php echo $user_id; ?></small></h1>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-lg-4">
                        ...
                        <!-- Emergency Contact (if member) -->
                        <?php if (($user['user_type'] ?? '') == 'member' && isset($user['emergency_name']) && $user['emergency_name']): ?>
                        <div class="card mt-4">
                            <div class="card-header">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <ul class="nav nav-tabs card-header-tabs" id="userTabs" role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link" id="membership-tab" data-toggle="tab" href="#membership" role="tab">Membership</a>
                                    </li>
                                    <li class="nav-item">
                                    ...
                                    <?php elseif (($user['user_type'] ?? '') == 'trainer'): ?>
                                    <li class="nav-item">
                                        <a class="nav-link" id="trainer-tab" data-toggle="tab" href="#trainer" role="tab">Trainer Details</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="assigned-classes-tab" data-toggle="tab" href="#assigned-classes" role="tab">Assigned Classes</a>
                                    </li>
                                    <?php endif; ?>
                                </ul>
                            </div>
                            <div class="card-body">
                                <div class="tab-content">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="info-row">
                                                    ...
                                                    <div class="info-value"><?php echo ucfirst($user['user_type'] ?? 'User'); ?></div>
                                                </div>
                                                <div class="info-row">
                                                    <div class="info-label">Status:</div>
                                                    <div class="info-value"><span class="badge badge-<?php echo ($user['status'] ?? '') == 'active' ? 'success' : 'danger'; ?>"><?php echo ucfirst($user['status'] ?? 'Inactive'); ?></span></div>
                                                </div>
                                                <div class="info-row">
                                                    <div class="info-label">Joined:</div>
                                                    <div class="info-value"><?php echo isset($user['created_at']) ? date('F d, Y', strtotime($user['created_at'])) : 'N/A'; ?></div>
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($recent_attendance as $att): ?>
                                                            <tr>
                                                                ...
                                                                <td><?php echo date('H:i', strtotime($att['check_in'])); ?></td>
                                                                <td><?php echo $att['check_out'] ? date('H:i', strtotime($att['check_out'])) : '-'; ?></td>
                                                                <td><span class="badge badge-info"><?php echo ucfirst($att['type']); ?></span></td>
                                                            </tr>
                                                        <?php endforeach; ?>
                                                    </tbody>
                                                </table>
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($recent_bookings as $booking): ?>
                                                            <tr>
                                                                ...
                                                                <td><span class="badge badge-<?php echo $booking['status'] == 'booked' ? 'warning' : ($booking['status'] == 'attended' ? 'success' : 'danger'); ?>"><?php echo ucfirst($booking['status']); ?></span></td>
                                                            </tr>
                                                        <?php endforeach; ?>
                                                    </tbody>
                                                </table>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="info-row">
                                                    ...
                                                    <div class="info-value"><?php echo htmlspecialchars($user['specialization'] ?? 'Not specified'); ?></div>
                                                </div>
                                                <div class="info-row">
                                                    <div class="info-label">Experience:</div>
                                                    <div class="info-value"><?php echo isset($user['experience_years']) ? $user['experience_years'] . ' years' : 'N/A'; ?></div>
                                                </div>
                                                <div class="info-row">
                                                    <div class="info-label">Qualification:</div>
                                                    <div class="info-value"><?php echo nl2br(htmlspecialchars($user['qualification'] ?? 'N/A')); ?></div>
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="info-row">
                                                    <div class="info-label">Joined:</div>
                                                    <div class="info-value"><?php echo isset($user['joining_date']) ? date('F d, Y', strtotime($user['joining_date'])) : 'N/A'; ?></div>
                                                </div>
                                                <div class="info-row">
                                                    <div class="info-label">Salary:</div>
                                                    <div class="info-value"><?php echo isset($user['salary']) ? '$' . number_format($user['salary'], 2) : 'N/A'; ?></div>
                                                </div>
                                                <div class="info-row">
                                                    <div class="info-label">Certification:</div>
                                                    <div class="info-value"><?php echo htmlspecialchars($user['certification'] ?? 'N/A'); ?></div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="tab-pane fade" id="assigned-classes" role="tabpanel">
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($assigned_classes as $class): ?>
                                                            <tr>
                                                                ...
                                                                <td><span class="badge badge-<?php echo $class['difficulty'] == 'beginner' ? 'success' : ($class['difficulty'] == 'intermediate' ? 'warning' : 'danger'); ?>"><?php echo ucfirst($class['difficulty']); ?></span></td>
                                                                <td><span class="badge badge-<?php echo $class['status'] == 'active' ? 'success' : 'secondary'; ?>"><?php echo ucfirst($class['status']); ?></span></td>
                                                                <td><a href="classes/view_class.php?id=<?php echo $class['class_id']; ?>" class="btn btn-sm btn-primary">View</a></td>
                                                            </tr>
                                                        <?php endforeach; ?>
                                                    </tbody>
                                                </table>
